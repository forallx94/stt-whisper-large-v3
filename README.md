...
`input/` 폴더의 음성 파일을 [openai/whisper-large-v3](https://huggingface.co/openai/whisper-large-v3)
로 전사하여 `output/` 폴더에 txt 파일로 저장합니다.

## 요구 사항

- Docker / Docker Compose
- NVIDIA GPU + 최신 드라이버, NVIDIA Container Toolkit (없으면 CPU로 동작하나 매우 느립니다)
...
이미 전사된 파일은 건너뜁니다. 다시 전사하려면 `--force`를 붙입니다.

## 출력

입력 파일 하나당 두 개의 txt가 생성됩니다.

| 파일 | 내용 |
| --- | --- |
| `output/<이름>.txt` | 전사 결과 전문 (타임스탬프 없음) |
| `output/<이름>.segments.txt` | 문장별 `[hh:mm:ss -> hh:mm:ss]` 타임스탬프 포함 |
...
- 모델 가중치(약 3GB)는 `hf-cache` 볼륨에 캐시되어 최초 실행에서만 내려받습니다.

## 참고

- 지원 확장자: m4a, mp3, wav, flac, ogg, opus, aac, wma, mp4, webm (ffmpeg가 읽을 수 있는 형식).
- GPU가 보이지 않으면 CPU로 동작하지만 매우 느립니다. 실행 로그 첫 줄에서 `GPU:` 표시를 확인하세요.
### 모델 카드 권장값에서 의도적으로 벗어난 두 가지

whisper-large-v3 모델 카드는 long-form 전사에 `condition_on_prev_tokens=True`와
`no_speech_threshold=0.6`을 권장하지만, 이 녹음들에서는 두 값 모두 문제를 일으켜 껐습니다.
...
전사 결과를 검증할 때는 `.segments.txt`의 마지막 타임스탬프가 헤더의 `duration`에
가까운지, 그리고 고유 세그먼트 비율이 지나치게 낮지 않은지 확인하면 위 두 실패를 잡을 수 있습니다.
